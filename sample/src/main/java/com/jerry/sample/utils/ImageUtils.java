...
public class ImageUtils {
	...
	private static final String PHOTO_COMPRESS_FILE_NAME = "compress_photo.jpg";
	...
	// 质量压缩方法
	public Bitmap compressImageByQuality(Bitmap image) {
	...
	public Bitmap compressImageByScaleWithPath(String srcPath) {
		...
		// 压缩好比例大小后再进行质量压缩
		return compressImageByQuality(bitmap);
	}

	// 图片按比例大小压缩方法（根据Bitmap图片压缩）
	public Bitmap compressImageByScaleWithBitmap(Bitmap image) {
	...
	public File bitmap2File(Bitmap bitmap) {
		File saveFile = new File(StorageUtils.getInstance().getImageDir(),
				PHOTO_COMPRESS_FILE_NAME);
	...
	public static Bitmap cropImage(Bitmap bitmap, int resizeWidth, int resizeHeight, boolean cropCenterFlag) {
        Config config = bitmap.getConfig();
        if (config == null)
            config = Config.ARGB_8888;
        Bitmap resizeBitmap = Bitmap.createBitmap(resizeWidth, resizeHeight, config);
        Canvas canvas = new Canvas(resizeBitmap);

        RectF destination = new RectF(0, 0, resizeWidth, resizeHeight);
        float ratio;
        float xratio = (float)resizeWidth / (float)bitmap.getWidth();
        float yratio = (float)resizeHeight / (float)bitmap.getHeight();
        if (cropCenterFlag)
            ratio = Math.max(xratio, yratio);
        else
            ratio = Math.min(xratio, yratio);

        float postWidth = bitmap.getWidth() * ratio;
        float postHeight = bitmap.getHeight() * ratio;
	...
	public static Bitmap getBitmapFormRes(Resources res, int resId) {
		Bitmap bitmap = BitmapFactory.decodeResource(res,resId);
		return bitmap;
	}
	
	public static Bitmap getBitmapFormSdcard(String filePath) {
