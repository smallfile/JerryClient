...
package com.jerry.uilib.widget.MaterialCircleProgressBar;
 ...
 * platforms.
 */
public class MyMaterialCircleProgressBar extends AppCompatImageView {
    ...
    public MyMaterialCircleProgressBar(Context context) {
        super(context);
        init(context, null, 0);

    }

    public MyMaterialCircleProgressBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0);

    }

    public MyMaterialCircleProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        final TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.MyMaterialCircleProgressBar, defStyleAttr, 0);
        final float density = getContext().getResources().getDisplayMetrics().density;

        mBackGroundColor = a.getColor(
                R.styleable.MyMaterialCircleProgressBar_circle_progressbar_background_color, DEFAULT_CIRCLE_BG_LIGHT);

        mProgressColor = a.getColor(
                R.styleable.MyMaterialCircleProgressBar_circle_progressbar_progress_color, DEFAULT_CIRCLE_BG_LIGHT);
        mColors = new int[]{mProgressColor};

        mInnerRadius = a.getDimensionPixelOffset(
                R.styleable.MyMaterialCircleProgressBar_circle_progressbar_inner_radius, -1);

        mProgressStokeWidth = a.getDimensionPixelOffset(
                R.styleable.MyMaterialCircleProgressBar_circle_progressbar_progress_stoke_width, (int) (STROKE_WIDTH_LARGE * density));
        mArrowWidth = a.getDimensionPixelOffset(
                R.styleable.MyMaterialCircleProgressBar_circle_progressbar_arrow_width, -1);
        mArrowHeight = a.getDimensionPixelOffset(
                R.styleable.MyMaterialCircleProgressBar_circle_progressbar_arrow_height, -1);
        mTextSize = a.getDimensionPixelOffset(
                R.styleable.MyMaterialCircleProgressBar_circle_progressbar_progress_text_size, (int) (DEFAULT_TEXT_SIZE * density));
        mTextColor = a.getColor(
                R.styleable.MyMaterialCircleProgressBar_circle_progressbar_progress_text_color, Color.BLACK);

        mShowArrow = a.getBoolean(R.styleable.MyMaterialCircleProgressBar_circle_progressbar_show_arrow, false);
        mCircleBackgroundEnabled = a.getBoolean(R.styleable.MyMaterialCircleProgressBar_circle_progressbar_enable_circle_background, true);


        mProgress = a.getInt(R.styleable.MyMaterialCircleProgressBar_circle_progressbar_progress, 0);
        mMax = a.getInt(R.styleable.MyMaterialCircleProgressBar_circle_progressbar_max, 100);
        int textVisible = a.getInt(R.styleable.MyMaterialCircleProgressBar_circle_progressbar_progress_text_visibility, 1);
        if (textVisible != 1) {
            mIfDrawText = true;
        }
    ...
    private class OvalShadow extends OvalShape {
        ...
        public void draw(Canvas canvas, Paint paint) {
            final int viewWidth = MyMaterialCircleProgressBar.this.getWidth();
            final int viewHeight = MyMaterialCircleProgressBar.this.getHeight();
