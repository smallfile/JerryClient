...
public class MonthView extends LinearLayout {
  ...
  public static MonthView create(ViewGroup parent, LayoutInflater inflater,
    ...
    for (int offset = 0; offset < 7; offset++) {
      ...
      String weekDayName = weekdayNameFormat.format(today.getTime());
      textView.setText(weekDayName.substring(1));
    }
  ...
  public void init(MonthDescriptor month, List<List<MonthCellDescriptor>> cells,
    ...
    title.setTextColor(this.getResources().getColor(R.color.calendar_text_month));
    ...
    for (int i = 0; i < 6; i++) {
      ...
      if (i < numRows) {
        ...
        for (int c = 0; c < week.size(); c++) {
          ...
          // 选中的单元格上面的文字设为忙
          if (cell.isSelected()) {
        	  	cellView.setText(R.string.calendar_text_selected);
          }
          
          // 如果日期小于当前，则不可点击
          if (cell.getDate().before(new Date())) {
